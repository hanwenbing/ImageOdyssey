...
function spawnCodex(prompt: string): Promise<CodexResult> {
...
export async function recommend(request: RecommendRequest): Promise<RecommendResponse> {
  ...
  const { stdout } = await spawnCodex(buildRecommendPrompt(request, canonicalCases));
...
export async function rewrite(request: RewriteRequest): Promise<RewriteResponse> {
  ...
  const { stdout } = await spawnCodex(buildRewritePrompt(request, caseRecord));
